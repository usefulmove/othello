;;; cadr-test.el --- Unit tests for Cadr functional programming -*- lexical-binding: t; -*-
;;
...
;;; Commentary:
;;
;;  Description: Cadr unit tests
;;
;;  Source code: ~/repos/cadr/src/cadr.el
;;
;;; Code:

(load-file "~/repos/cadr/src/cadr.el") ; load Cadr language


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; test definitions

(defun cadr-test-compound (error-prelude)
...
(defun cadr-test-compound2 (error-prelude)
  (assert-equal
...
(defun cadr-test-compound3 (error-prelude)
...
(defun cadr-test-function-composition (error-prelude)
...
(defun cadr-test-function-composition2 (error-prelude)
  (when (not= (thread 5
                'sqrt
                (\ (a) (- a 1))
                (\ (a) (/ a 2)))
...
(defun cadr-test-string-join (error-prelude)
  (assert-equal
    (join '("3" "1" "2" "5" "4"))
    "31254"
...
(defun cadr-test-curry (error-prelude)
  (letrec ((square (\ (a) (* a a)))
...
(defun cadr-test-partial (error-prelude)
  (letrec ((square (\ (a) (* a a)))
           (sum-squares (lambda (a b)
                          (sqrt (+ (call square a)
                                   (call square b))))))
...
(defun cadr-test-foldl (error-prelude)
  (assert-equal
...
(defun cadr-test-foldr (error-prelude)
  (let ((test-list '(3 1 2 5 4)))
    (assert-equal
      (foldr
        (lambda (acc a) (cons a acc))
...
(defun cadr-test-drop-take (error-prelude)
  (assert-equal
    (take 3 '(3 1 2 0 5 4))
    '(3 1 2)
...
(defun cadr-test-zip (error-prelude)
  (assert-equal
    (zip '(3 1 2 5 4)
         '(0 1 2 3))
    '((3 . 0) (1 . 1) (2 . 2) (5 . 3))
...
(defun cadr-test-enumerate-partition (error-prelude)
  (assert-equal
    (enumerate '(3 1 2 5 4))
    '((0 . 3) (1 . 1) (2 . 2) (3 . 5) (4 . 4))
...
(defun cadr-test-tally (error-prelude)
  (assert-equal
...
(defun cadr-test-for-comprehension (error-prelude)
  (assert-equal
...
(defun cadr-test-equality (error-prelude)
  (assert-equal
    (not= 1 1.0 1) ; nil
    (eqp 1 1.0) ; nil
...
(defun cadr-test-chars (error-prelude)
  (assert-equal
    (char-to-int ?8)
    8
    (concat error-prelude "error: equality test(s) failed"))
...
(defun cadr-test-impure (error-prelude)
  (let ((n 0))
    (for-each
      (lambda (a)
        (setq n (+ n (* a a))))
...
(defun cadr-test-list-ref (error-prelude)
  (assert-equal
...
(defun cadr-test-logic (error-prelude)
  (assert-equal
   (truep t)
   t
...
(defun cadr-test-run-tests (&rest tests)
  (letrec ((prelude "cadr-test ... ")
...
(cadr-test-run-tests
  ...
  'cadr-test-compound2
  'cadr-test-compound3
  'cadr-test-function-composition
  'cadr-test-function-composition2
  'cadr-test-string-join
  'cadr-test-curry
  'cadr-test-partial
  'cadr-test-foldl
  'cadr-test-foldr
